...
def dump_data(filename: str = "dump.txt", total_number_of_anime: int = 50):
    ...
    print(f"Prenešeno v datoteko: {filename}")
...
def get_more_data(csv_file_name: str = "anime_data.csv", output_file: str = "anime_data_premium.csv"):
    ...
    with open(output_file, mode="w", encoding=encoding, newline="") as csv_out:
        ...
        writer.writerow(["Rank", "Score", "Name", "ID", "Link", "Type", "Episodes", "Start Date", "End Date", "Status", "Premiered", "Broadcast", "Studios", "Source", "Genres", "Demographics", "Duration", "Rating", "Popularity", "Members", "Favorites", "Aired start", "Aired end"])
    csv_out.close()
    ...
    with open(csv_file_name, mode="r", encoding=encoding) as csv_file:
        ...
        for i, row in enumerate(reader):
            ...
            try:
                ...
                block = re.search(r'<span[^>]*class="dark_text"[^>]*>\s*Genres?:\s*</span>(.*?)</div>', content, flags=re.S | re.I)
                if block:
